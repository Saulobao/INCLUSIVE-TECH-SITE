...
import { Moon, Home, Bell, Activity, Settings, Monitor, ShieldAlert, Baby } from "lucide-react";
    ...
    <div className="flex h-screen overflow-hidden bg-background text-foreground">
      ...
      <aside className="w-64 border-r border-border bg-card flex flex-col">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          ...
          <NavItem href="/events" icon={Activity} label="Events" active={location === "/events"} />
          <NavItem href="/settings" icon={Settings} label="Settings" active={location === "/settings"} />
        </nav>

        <div className="p-4 border-t border-border">
          <div className={cn("rounded-lg p-3 flex items-center gap-3", isOnline ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive")}>
            <div className="relative">
              <Monitor className="h-5 w-5" />
              <span className={cn("absolute -bottom-1 -right-1 h-2.5 w-2.5 rounded-full border-2 border-card", isOnline ? "bg-green-500" : "bg-destructive")} />
            </div>
            <div>
